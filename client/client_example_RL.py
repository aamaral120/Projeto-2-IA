...
#andar aleatoriamente ate chegar ao objetivo (ou target)
#    guardar o caminho percorrido [(posiçao_anterior posição_posterior, ação, reward),...]

#percorrer lista ao contrario, para cada tuplo:
#    atualizar a qtable: q[posicao_anterior][ação] = reward + gamma *  max(qtable[posição_posterior].values())

#para cada chave da qtable (x,y):
#    fazer "marrow", a direçao é max(qtable[(x,y)], key=qtable[(x,y)].get)